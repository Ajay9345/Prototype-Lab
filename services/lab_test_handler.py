...
class LabTestHandler:
    ...
    def process_lab_report(self, image_file, user_id: str, gender: str = "male") -> Dict:
        ...
        return {
            "id": report_id, "filename": image_file.filename, "image_path": image_path,
            "upload_date": datetime.now().isoformat(), "extracted_text": text,
            "test_category": self.get_test_category(test_values), "test_values": test_values, "gender": gender,
        }

    def parse_test_values(self, text: str) -> Dict:
        parsed = {}
        if not text:
            return parsed
        for line in text.split("\n"):
            ...
            for pattern, test_name in TEST_PATTERNS.items():
                if re.search(pattern, line_lower):
                    if match := re.search(VALUE_PATTERN, line):
                        parsed[test_name] = {"value": float(match.group(1)), "unit": match.group(2) or "", "raw_line": line.strip()}
                    break
        return parsed
        ...
        for ext in [".jpg", ".jpeg", ".png", ".gif", ".bmp", ".pdf"]:
            ...
            if os.path.exists(path):
                try:
                    ...
                    print(f"Error deleting lab report file: {e}"); return False
        return False
    ...
    def _load_normal_ranges(self) -> Dict:
        try:
            ...
            print(f"Error loading normal ranges: {e}"); return {}

    def _extract_text(self, image_path: str) -> str:
        try:
            return pytesseract.image_to_string(Image.open(image_path), config="--oem 3 --psm 6").strip()
        except Exception as e:
