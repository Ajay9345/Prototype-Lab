...
# Keywords that suggest the user is in distress (includes Hindi words)
DISTRESS_KEYWORDS = [
...
# ── Static hospital database (major Indian hospitals) ─────────────────────────
HOSPITALS: List[Dict] = [
...
# ── Static 24/7 pharmacy database ─────────────────────────────────────────────
# Covers major cities across India with multiple entries per city
...
# Type-specific first-aid instructions added to emergency guidance
EMERGENCY_TYPE_INSTRUCTIONS: Dict[str, List[str]] = {
    "heart|chest": [
...
MAX_DISTANCE_KM = 200.0   # Search within 200 km


class EmergencySupport:
    """
    Provides emergency detection, hospital/pharmacy lookup,
    and step-by-step emergency guidance.
    """

    def detect_distress_keywords(self, text: str) -> bool:
        """Return True if the text contains any distress keyword."""
        text_lower = text.lower()
        return any(kw in text_lower for kw in DISTRESS_KEYWORDS)

    def get_emergency_contacts(self) -> Dict[str, str]:
        """Return the dictionary of emergency phone numbers."""
        return EMERGENCY_CONTACTS

    def get_emergency_guidance(self, emergency_type: Optional[str] = None) -> Dict:
        """
        Return general emergency guidance, optionally enriched with
        type-specific instructions (e.g. for heart, bleeding, burns).
        """
        guidance = {
            "primary_number": "108",
        ...
        # Add type-specific instructions if an emergency type is given
        if emergency_type:
            etype = emergency_type.lower()
            for pattern, instructions in EMERGENCY_TYPE_INSTRUCTIONS.items():
                # Pattern may contain '|' for multiple keywords (e.g. "heart|chest")
    ...
    def find_nearby_hospitals(
        ...
        """
        Return up to `max_results` hospitals within MAX_DISTANCE_KM,
        sorted by distance from the given coordinates.
        """
        return self._find_nearby(HOSPITALS, latitude, longitude, max_results)
    ...
    def find_nearby_pharmacies(
        ...
        """
        Return up to `max_results` 24/7 pharmacies within MAX_DISTANCE_KM,
        sorted by distance.
        """
        open_pharmacies = [p for p in PHARMACIES if p.get("open_24_7")]
        return self._find_nearby(open_pharmacies, latitude, longitude, max_results)
    ...
    def _find_nearby(
        ...
        """Filter by distance, sort, and return top N.
        If nothing falls within MAX_DISTANCE_KM, return the closest N anyway."""
        scored = []
        ...
        within_range = [e for e in scored if e["distance_km"] <= MAX_DISTANCE_KM]
        results = within_range if within_range else scored  # fallback: nearest regardless
...
def get_emergency_support() -> EmergencySupport:
    """Return the shared EmergencySupport instance."""
    global _instance
