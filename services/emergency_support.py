...
class EmergencySupport:
    ...
    def find_nearby_hospitals(self, latitude: float, longitude: float, max_results: int = 5) -> List[Dict]:
        return self._find_nearby(HOSPITALS, latitude, longitude, max_results)

    def find_nearby_pharmacies(self, latitude: float, longitude: float, max_results: int = 5) -> List[Dict]:
    ...
    def _find_nearby(self, locations: List[Dict], lat: float, lon: float, max_results: int) -> List[Dict]:
        scored = []
        ...
        results = within_range if within_range else scored
